...
# ==================== 3. 修复 hostapd ap_isolate 问题 ====================
echo "=== 修复 hostapd ap_isolate 问题 ==="

# 定义 hostapd.sh 路径（兼容 feeds 和 package 目录）
HOSTAPD_SH=""
...
if [ -n "$HOSTAPD_SH" ] && [ -f "$HOSTAPD_SH" ]; then
    if grep -q 'append bss_conf "ap_isolate=' "$HOSTAPD_SH"; then
        ...
        echo "✗ 未找到 ap_isolate 配置行，可能代码结构已变化"
        exit 1
    fi
else
    echo "⚠ 未找到 hostapd.sh 文件，跳过 ap_isolate 修复"
fi
...
' "$MAC80211_SH"
    ...
    echo "⚠ 未找到 mac80211.sh 文件，跳过无线配置修改"
fi

# ==================== 5. 可选：创建 ap_isolate 修复的开机脚本（备用） ====================
cat > files/etc/uci-defaults/99-fix-ap-isolate << 'EOF'
#!/bin/sh
# 备用方案：如果 hostapd 配置中仍然有 ap_isolate=1，强制修复
sleep 5
if grep -q "ap_isolate=1" /var/run/hostapd-phy*.conf 2>/dev/null; then
    ...
    logger -t "fix-ap-isolate" "已修复 ap_isolate 配置"
fi
exit 0
EOF
chmod +x files/etc/uci-defaults/99-fix-ap-isolate
echo "✅ 创建 ap_isolate 修复开机脚本（备用）"
